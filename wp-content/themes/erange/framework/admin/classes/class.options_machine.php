...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				//drag & drop slidebar manager
				case 'sidebar':
					$output .= '<div class="slider"><ul id="'.$value['id'].'">';
					$sidebar_item = $smof_data[$value['id']];
					$count = count($sidebar_item);
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						...
						$i = 0;
						foreach ($sidebar_item as $sidebari) {
							$oldorder = $sidebari['order'];
							$i++;
							$order = $i;
							$output .= Options_Machine::optionsframework_sidebar_function($value['id'],$value['std'],$oldorder,$order);
						}
	...
	public static function optionsframework_sidebar_function($id,$std,$oldorder,$order){
		...
		$sidebar_item = array();
		if (isset($smof_data[$id]))
	    	$sidebar_item = $smof_data[$id];
		
	    if (isset($sidebar_item[$oldorder])) { $val = $sidebar_item[$oldorder]; } else {$val = $std;}
		
		//initialize all vars
		$sidebar_itemvars = array('name','id','description');
		
		foreach ($sidebar_itemvars as $sidebar_itemvar) {
			if (!isset($val[$sidebar_itemvar])) {
				$val[$sidebar_itemvar] = '';
			}
		}
		...
		//begin slider interface	
		if (!empty($val['name'])) {
			$sidebar .= '<li><div class="slide_header"><strong>'.stripslashes($val['name']).'</strong>';
		} else {
		...
		$sidebar .= '<label>Name</label>';
		$sidebar .= '<input class="slide of-input of-slider-name" name="'. $id .'['.$order.'][name]" id="'. $id .'_'.$order .'_slide_name" value="'. stripslashes($val['name']) .'" />';
		
		$sidebar .= '<label>ID</label>';
		$sidebar .= '<input class="slide of-input of-slider-id" name="'. $id .'['.$order.'][id]" id="'. $id .'_'.$order .'_slide_id" value="'. stripslashes($val['id']) .'" />';

		$sidebar .= '<label>Description (optional)</label>';
		$sidebar .= '<textarea class="slide of-input" name="'. $id .'['.$order.'][description]" id="'. $id .'_'.$order .'_slide_description" cols="8" rows="8">'.stripslashes($val['description']).'</textarea>';
